...
class VK:
    ...
    def get_photos(self, user_id, count = 5):
        ...
        params = {'owner_id': user_id,'count': count,'album_id': album_combobox.get(),'extended': 1}
...
class Counter_program():
    ...
    def create_widgets(self):
        ...
        vk_frame = ttk.LabelFrame(self.window, text="Исходные данные для запроса ВКонтакте", relief=tk.RIDGE)
        vk_frame.grid(row=1, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        token_label = ttk.Label(vk_frame, text="Ключ к API:")
        token_label.grid(row=1, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        global token_entry
        token_entry = ttk.Entry(vk_frame, width=100)
        token_entry.grid(row=1, column=2, sticky=tk.W, padx=5, pady=3)
        token_entry.insert(tk.END, "")

        id_label = ttk.Label(vk_frame, text="ID пользователя:")
        id_label.grid(row=2, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        global id_entry
        id_entry = ttk.Entry(vk_frame, width=50)
        id_entry.grid(row=2, column=2, sticky=tk.W, padx=5, pady=3)
        id_entry.insert(tk.END, "")

        album_label = ttk.Label(vk_frame, text="Выберите альбом:")
        album_label.grid(row=3, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        self.combobox_value = tk.StringVar()
        global album_combobox
        album_combobox = ttk.Combobox(vk_frame, height=4, textvariable=self.combobox_value, state="readonly")
        album_combobox.grid(row=3, column=2, sticky=tk.W, padx=5, pady=3)
        album_combobox['values'] = ("wall", "profile", "saved")
        album_combobox.current(1)

        count_foto_label = ttk.Label(vk_frame, text="Количество фото в запросе:")
        count_foto_label.grid(row=4, column=1, sticky=tk.W + tk.N, padx=5, pady=5)
        ...
        count_foto_spinbox.grid(row=4, column=2, sticky=tk.W, padx=5, pady=5)
        count_foto_spinbox.delete(0, tk.END)
        count_foto_spinbox.insert(1, "5")
        count_foto_spinbox.configure(validate="key", validatecommand=vcmd)

        vk_button = tk.Button(vk_frame, text="Отправить запрос", command=self.get_vk_info)
        vk_button.grid(row=4, column=2, padx=5, pady=5, sticky=tk.E)
        ...
        vk_info_frame.grid(row=1, column=2, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)
        ...
        vk_info_label.grid(row=2, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        global vk_foto_frame
        vk_foto_frame = ttk.LabelFrame(self.window, text="ФОТО из ВКонтакте", relief=tk.RIDGE)
        vk_foto_frame.grid(row=2, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        global vk_foto_name
        vk_foto_name = ttk.LabelFrame(vk_foto_frame, text="Имя файла ФОТО", relief=tk.RIDGE)
        vk_foto_name.grid(row=1, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        vk_prev_button = tk.Button(vk_foto_name, text="ФОТО <<<", command=self.get_prev_foto)
        vk_prev_button.grid(row=1, column=1, padx=5, pady=5)

        vk_next_button = tk.Button(vk_foto_name, text=">>> ФОТО", command=self.get_next_foto)
        vk_next_button.grid(row=1, column=2, padx=5, pady=5)

        vk_choice_button = tk.Button(vk_foto_name, text="Выбрать текущее ФОТО", command=self.get_choice_foto)
        vk_choice_button.grid(row=4, column=1, padx=5, pady=5, columnspan=2)

        vk_choice_button = tk.Button(vk_foto_name, text="Выбрать все ФОТО", command=self.get_choice_all_foto)
        vk_choice_button.grid(row=5, column=1, padx=5, pady=5, columnspan=2)

        global vk_foto
        vk_foto = ttk.LabelFrame(vk_foto_frame, text="Миниатюра текущего ФОТО", relief=tk.RIDGE)
        vk_foto.grid(row=1, column=2, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)
        ...
        vk_foto_listbox.grid(row=2, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5, columnspan=2)
        ...
        yd_foto_frame.grid(row=2, column=2, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        global yd_foto_listbox
        yd_foto_listbox = tk.Listbox(yd_foto_frame, selectmode='single', width=30)
        yd_foto_listbox.grid(row=1, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        yd_del_button = tk.Button(yd_foto_frame, text="Удалить из списка", command=self.del_foto)
        yd_del_button.grid(row=2, column=1, padx=5, pady=5)
        ...
        yd_frame.grid(row=4, column=1, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)

        yd_token_label = ttk.Label(yd_frame, text="Ключ авторизации:")
        yd_token_label.grid(row=1, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        global yd_token_entry
        yd_token_entry = ttk.Entry(yd_frame, width=100)
        yd_token_entry.grid(row=1, column=2, sticky=tk.W, padx=5, pady=3)
        yd_token_entry.insert(tk.END, "")

        yd_path_label = ttk.Label(yd_frame, text="Папка на Яндекс.Диске:")
        yd_path_label.grid(row=2, column=1, sticky=tk.W + tk.N, padx=5, pady=5)

        global yd_path_entry
        yd_path_entry = ttk.Entry(yd_frame, width=50)
        yd_path_entry.grid(row=2, column=2, sticky=tk.W, padx=5, pady=3)
        yd_path_entry.insert(tk.END, "ФОТО из ВК")

        yd_button = tk.Button(yd_frame, text="Загрузить на Яндекс.Диск", command=self.upload)
        yd_button.grid(row=3, column=2, padx=5, pady=5, sticky=tk.E)
        ...
        log_frame.grid(row=4, column=2, sticky=tk.E + tk.W + tk.N + tk.S, padx=5, pady=5)
        ...
        log_button.grid(row=2, column=1, padx=5, pady=5, sticky=tk.E)
    ...
    def save_log(self):
        if len(log_text.get('1.0', tk.END)) > 1:
            ...
            with open(f'log/{log_file_name}.txt', 'w') as f:
                for log_str in log_text.get('1.0', tk.END):
                    f.write(log_str)
    ...
    def set_log_string(self, log_string):
        log_text.insert(tk.END, log_string)
        log_text.see(tk.END)
        return

    # Функция обработки нажатия кнопки - предыдущее фото
    def get_prev_foto(self):
        if vk_foto_listbox.index('active') > 0:
            getval_index = vk_foto_listbox.index('active')
            vk_foto_listbox.select_clear(getval_index)
            vk_foto_listbox.select_set(getval_index - 1)
            vk_foto_listbox.activate(getval_index - 1)
            getval_text = vk_foto_listbox.get('active')
            self.get_foto(getval_text)

    # Функция обработки нажатия кнопки - следующее фото
    def get_next_foto(self):
        if vk_foto_listbox.index('active') < vk_foto_listbox.size() - 1:
            getval_index = vk_foto_listbox.index('active')
            vk_foto_listbox.select_clear(getval_index)
            vk_foto_listbox.select_set(getval_index + 1)
            vk_foto_listbox.activate(getval_index + 1)
    ...
    def get_choice_foto(self):
        if vk_foto_listbox.index('active') >= 0:
            if vk_foto_listbox.get('active') not in yd_foto_listbox.get(0, tk.END):
                yd_foto_listbox.insert(tk.END, vk_foto_listbox.get('active'))
                yd_foto_listbox.select_set(tk.END)
                yd_foto_listbox.activate(tk.END)
        return

    # Функция обработки нажатия кнопки - добавить все фото в список загрузки
    def get_choice_all_foto(self):
        if vk_foto_listbox.size() > 0:
            for i in range(len(vk_foto_listbox.get(0, tk.END))):
                yd_foto_listbox.insert(tk.END, vk_foto_listbox.get(0, tk.END)[i])
            yd_foto_listbox.select_set(tk.END)
            yd_foto_listbox.activate(tk.END)
        return
    ...
    def del_foto(self):
        if len(yd_foto_listbox.get(0, tk.END)) > 0 and yd_foto_listbox.index('active') >= 0:
            yd_foto_listbox.delete('active')
            yd_foto_listbox.select_set(tk.END)
            yd_foto_listbox.activate(tk.END)
        return
    ...
    def get_foto(self, file_name):
        ...
        vk_foto_label.config(image='')
        vk_foto_label.config(image=tatras)

        vk_foto_label.image = tatras
        vk_foto_label.pack()

        vk_foto_filename_label.config(text=file_name)
        vk_foto_filename_label.pack()
    ...
    def get_vk_info(self):
        ...
        self.set_log_string(f'ID пользователя в ВК: {id_entry.get()}\n')

        # Создаем экземпляр объекта класса VK
        vk = VK(token_entry.get())

        # Сохраняем результат запроса информации о профиле в список
        dict_user_info = vk.users_info(id_entry.get())
        ...
        vk_info_label.config(text=(f'ID пользователя: {dict_user_info['response'][0]['id']}\n'
        ...
        if dict_user_info['response'][0]['can_access_closed']:
            # Сохраняем результат запроса информации о профиле в список
            dict_user_foto = vk.get_photos(id_entry.get(), count=count_foto_spinbox.get())
            ...
            if 'error' not in dict_user_foto:
                ...
                if dict_user_foto['response']['count'] > 0:
                    ...
                    self.set_log_string(f'В запросе: {count_foto_spinbox.get()} фотографий\n')
                    vk_foto_frame.config(text=f'ФОТО из ВКонтакте - Альбом: "{album_combobox.get()}". '
                                              f'Количество фото в профиле: {dict_user_foto['response']['count']}, '
                                              f'запрошено {count_foto_spinbox.get()} фото')
                    list_url = []
                    list_filename = []
                    vk_foto_listbox.delete(0, tk.END)
                    yd_foto_listbox.delete(0, tk.END)
                    yd_foto_listbox.activate(tk.END)
                    ...
                    for dict in dict_user_foto['response']['items']:
                        ...
                        if 'orig_photo' not in dict:
                            ...
                            for size in dict['sizes']:
                                if size['type'] == 'w':
                                    size_list[0] = size['url']
                                elif size['type'] == 'z':
                                    size_list[1] = size['url']
                                elif size['type'] == 'y':
                                    size_list[2] = size['url']
                                elif size['type'] == 'x':
                                    size_list[3] = size['url']
                                elif size['type'] == 'r':
                                    size_list[4] = size['url']
                                elif size['type'] == 'q':
                                    size_list[5] = size['url']
                                elif size['type'] == 'p':
                                    size_list[6] = size['url']
                                elif size['type'] == 'o':
                                    size_list[7] = size['url']
                                elif size['type'] == 'm':
                                    size_list[8] = size['url']
                                elif size['type'] == 's':
                                    size_list[9] = size['url']
                        ...
                        if response.status_code == 200:
                            ...
                            vk_foto_listbox.insert(tk.END, my_filename)
                        else:
                            ...
                            dial.show_text()
                    vk_foto_listbox.select_set(0)
                    self.get_foto(list_filename[0])
                else:
                    vk_foto_frame.config(
                        text=f'ФОТО из ВКонтакте - Альбом: "{album_combobox.get()}". В выбранном альбоме НЕТ фото!')
            else:
                dial = MyDialog("FOTO_from_VK_to_YD",
    ...
    def upload(self):
        ...
        params = {'path': yd_path_entry.get()}
        headers = {'Authorization': f'OAuth {yd_token_entry.get()}'}
        ...
        for name in yd_foto_listbox.get(0, tk.END):
            # Формируем строку запроса
            upload_url = 'https://cloud-api.yandex.net/v1/disk/resources/upload'
            params = {'path': f'{yd_path_entry.get()}/{name}'}
            ...
            if response.status_code == 200:
                ...
                self.set_log_string(f'Фото: {name} загружено в папку {yd_path_entry.get()} на Яндекс.Диске\n')
                ...
                self.set_log_string(f'Ответ сервера [409]: {response.text}\n')
                dial = MyDialog("FOTO_from_VK_to_YD",
                                f'Фото {name } уже существует на сервере в папке {yd_path_entry.get()}')
                dial.show_text()
